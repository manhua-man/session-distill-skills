#!/usr/bin/env python3
"""Grok Deep Distill batch runner (shared paradigm)."""
...
def packet_path(session_id: str) -> Path:
    return mod.PACKETS_DIR / f"{session_id}.md"


def main() -> int:
    parser = argparse.ArgumentParser(description="Grok deep distill batch runner")
    ...
    print(f"==> Grok deep distill batch: {len(batch)} sessions (offset={args.offset})")
    for s in batch:
    ...
    batch_label = f"Grok batch offset {args.offset} ({len(batch)} sessions)"
