---
description: OpenCode Deep Distill — Grok 范式（3 条/批，answer-me 验证后晋升）
argument-hint: "[deep-distill | status | run --next N | mark <id> distilled]"
user-invocable: true
---

Use the **OpenCode Deep Distill** workflow (same paradigm as Grok / Cursor / Codex).

Always load:

- `~/.config/opencode/skills/session-distill/SKILL.md`
- `~/.config/opencode/skills/session-distill/references/deep-distill-workflow.md`
- `~/.config/opencode/skills/session-distill/references/distillation-rules.md`

## Mental model

```
storage JSON → packet → answer-packet (claims + verify) → session note → KB (ANSWERED only)
```
...
| `status` / `list` / `run` / `mark` | `opencode-session-distill.py <cmd>` |
...
python ~/.config/opencode/skills/session-distill/bin/deep-distill-run.py --offset <N> --batch-size 3
...
- `Coverage: partial` → read raw storage JSON before closing Q rows.
