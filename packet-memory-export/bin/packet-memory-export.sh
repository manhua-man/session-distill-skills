#!/bin/bash
# Packet Memory Exporter - Bash wrapper for Python script

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="$SCRIPT_DIR/packet-memory-export.py"
PYTHON_EXE=""
...
if [[ -z "$PYTHON_EXE" ]]; then
  echo "No Python interpreter found. Install Python or update packet-memory-export.sh." >&2
  exit 1
fi
