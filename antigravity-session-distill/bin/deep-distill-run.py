#!/usr/bin/env python3
"""Antigravity Deep Distill batch runner (shared paradigm)."""
...
SCRIPT = Path(__file__).resolve().parent / "antigravity-session-distill.py"
spec = importlib.util.spec_from_file_location("agy_sd", SCRIPT)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Antigravity deep distill batch runner")
    ...
    print(f"==> Antigravity deep distill batch: {len(batch)} sessions (offset={args.offset})")
    for s in batch:
    ...
    report = ddl.render_check_work_report(
        batch_label=f"Antigravity batch offset {args.offset}",
        session_ids=ids,
        promoted=[],
        not_promoted=[],
        verdict="PENDING",
